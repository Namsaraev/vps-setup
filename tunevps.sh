...
# ============================================
# ОПРЕДЕЛЕНИЕ MINIMIZED (4 признака)
# ============================================
detect_minimized() {
...
[ "$IS_MINIMIZED" = true ] && print_warning "Вариант: MINIMIZED ⚠️" || print_success "Вариант: Standard ✓"

# ============================================
# ВАЖНО: ОПРЕДЕЛЕНИЕ ПОЛЬЗОВАТЕЛЯ ДЛЯ НАСТРОЙКИ
# ============================================
# SUDO_USER может быть пустым при запуске через pipe (curl | bash)
# Поэтому спрашиваем пользователя явно
print_section "ОПРЕДЕЛЕНИЕ ПОЛЬЗОВАТЕЛЯ"

# Если SUDO_USER установлен и не root
if [ -n "$SUDO_USER" ] && [ "$SUDO_USER" != "root" ]; then
    CURRENT_USER="$SUDO_USER"
    print_info "Обнаружен пользователь: $CURRENT_USER (из SUDO_USER)"
else
    # Спрашиваем вручную
    echo ""
    echo "Для какого пользователя настраивать рабочее пространство?"
    echo "  1) root"
    echo "  2) ubuntu (стандартный пользователь Oracle Cloud)"
    echo "  3) pin (пользователь, создаваемый скриптом)"
    echo "  4) Другой пользователь"
    ask_input "Ваш выбор [1/2/3/4]: " USER_CHOICE
    
    case "$USER_CHOICE" in
        1) CURRENT_USER="root" ;;
        2) CURRENT_USER="ubuntu" ;;
        3) CURRENT_USER="pin" ;;
        4) 
...
part2_setup() {
    ...
    if [ "$IS_MINIMIZED" = true ]; then
        print_error "⚠️  MINIMIZED система! Рекомендуется сначала выполнить пункт 1"
    ...
    # 2.3 Базовые утилиты
    print_section "2.3 БАЗОВЫЕ УТИЛИТЫ"
    apt-get install -y \
    ...
    # 2.4 Zoxide (официальный установщик)
    print_section "2.4 ZOXIDE"
...
LIMITS
    ...
    # 2.9 Создание пользователя pin
    print_section "2.9 СОЗДАНИЕ ПОЛЬЗОВАТЕЛЯ $NEW_USER"
    ...
    # 2.10 SSH-ключ для pin
    print_section "2.10 SSH-КЛЮЧ ДЛЯ $NEW_USER"
...
HARD
    ...
    # 2.13-2.17 Zsh, Oh My Zsh, Powerlevel10k, плагины
    print_section "2.13 ZSH + OH MY ZSH + P10K"
...
setup_fzf() {
  ...
  for path in "/usr/share/doc/fzf/examples/key-bindings.zsh" "/usr/share/fzf/key-bindings.zsh" "/usr/local/share/fzf/key-bindings.zsh"; do
    if [[ -f "$path" ]]; then
      source "$path" 2>/dev/null
      [[ -f "${path%/*}/completion.zsh" ]] && source "${path%/*}/completion.zsh" 2>/dev/null
      return 0
    fi
  done
  local tmp
...
ZSHRC_TEMPLATE
    ...
    # Права доступа
    if [ "$CURRENT_USER" != "root" ]; then
        chown -R "$CURRENT_USER:$CURRENT_USER" "$USER_HOME/.oh-my-zsh" "$USER_HOME/.zshrc" 2>/dev/null || true
    fi
    chmod 644 "$USER_HOME/.zshrc"
    print_success ".zshrc создан для $CURRENT_USER"

    # ============================================
    # ФИНАЛЬНЫЙ ШАГ: ПЕРЕЗАПУСК ОБОЛОЧКИ
    # ============================================
    print_section "ФИНАЛ"
    echo ""
    print_success "Настройка завершена!"
    echo ""
    print_info "Чтобы применить изменения:"
    ...
    echo ""
    print_warning "Для запуска p10k configure прямо сейчас:"
    print_info "  source ~/.zshrc && p10k configure"
    echo ""
}
