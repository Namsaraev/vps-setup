...
# Функция интерактивного ввода (работает даже через pipe)
ask_input() {
    local prompt="$1"
    local var_name="$2"
...
# Функция паузы (не требует переменной)
pause() {
...
# Получаем список пользователей с оболочкой bash/zsh (кроме системных)
VALID_USERS=()
...
part2_setup() {
    ...
    # --- 2.5 Установка Zoxide (официальный способ — работает на всех архитектурах) ---
...
HARDENING_EOF
    ...
    if [ ! -d "$USER_HOME/.oh-my-zsh" ]; then
        ...
        if [ "$CURRENT_USER" = "root" ]; then
            ...
            # Убеждаемся что домашняя директория существует
            if [ ! -d "$USER_HOME" ]; then
                mkdir -p "$USER_HOME"
    ...
    # и setup_fzf НЕ добавляет файлы в PATH
    # ============================================
    TMP_ZSHRC=$(mktemp)
...
# Проверяем что PATH корректен (диагностика + fallback)
...
# ВАЖНО: функция source'ит файлы, НЕ добавляя их пути в PATH
setup_fzf() {
  ...
  # ВАЖНО: source'им файл, НЕ добавляем его путь в PATH!
  local fzf_paths=(
